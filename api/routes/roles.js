...
router.post("/add", async (req, res) => {
  ...
  try {
    ...
    let role = new Roles({role_name: body.role_name,is_active: true,created_by: req.user?.id,});
...
router.post("/update", async (req, res) => {
  ...
  try {
    ...
    if (body.permissions && Array.isArray(body.permissions) && body.permissions.length > 0) {
      ...
      let removedPermissions= permissions.filter(p=> !body.permissions.includes(p.permissions))
